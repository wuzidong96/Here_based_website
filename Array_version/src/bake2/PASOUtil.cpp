...
int 
PASOUtil::getShortestPath_cch(const TPt <TNodeEDatNet<PASONodeData, PASOEdgeData> >& PNet,
                          int source,
                          int sink,
                          const std::string& type,
                          std::vector<int>& path_o,
                          ...
                          double lambda)
{
    ...
    int edge_count = tail.size();
    unsigned u[edge_count];
    unsigned v[edge_count];
    int count = 0;
	for(std::vector<unsigned>::iterator m = tail.begin(); m != tail.end(); m++)
	{
		u[count]=*m;
		count+=1;
	}
    count = 0;
	for(std::vector<unsigned>::iterator m = head.begin(); m != head.end(); m++)
	{
		v[count]=*m;
		count+=1;
	}

    unsigned inf = std::numeric_limits<unsigned>::infinity();
	...
	if (PNet->IsEdge(src,des))
	{
        	if(type == "distance")
            	{
                    cost = PNet->GetEDat(src,des).getDistance();
            	}
            	else if(type == "time_min")
            	{
            	    cost = PNet->GetEDat(src,des).getTimeMin();
            	}
            	else if(type == "new_weight")
            	{
            	    cost = PNet->GetEDat(src,des).newWeightGivenLambda(lambda);
            	}
		else
            	{
                	std::cout << "wrong type" << std::endl;
		...
		weight.push_back(cost*1000);
		//cout << src <<' ' << des << ' ' << cost << ' ' << weight[weight.size()-1] << endl;
	}
	else
        {
		weight.push_back(inf);
		//cout << src <<' ' << des << ' ' << "inf" << endl;
	}
    }
	/*for(std::vector<unsigned>::iterator m = weight.begin(); m != weight.end(); m++ )
	{
		cout<<*m<<' ';
	}*/
    ...
    cout << "Customizing done [" << timer << "musec]" << endl;
    ...
    try{
        ...
        //cout << cch_query.get_distance() << endl;
	...
	/*
        ...
        time = -get_micro_time();
        auto node_path = cch_query.get_node_path();         //node or arc?
        time += get_micro_time();
	...
	*/
        ...
        throw std::runtime_error("during query "+std::to_string(0)+" : "+err.what());
    }

    auto path = cch_query.get_node_path();
    timer += get_micro_time();
    

    cout << "Query done [" << timer << "musec]" << endl;
    path_o.clear();
    for(std::vector<unsigned>::iterator m = path.begin(); m != path.end(); m++ )
    {
        path_o.push_back(*m);
	//cout << *m <<' ';
    }
...
int 
PASOUtil::writeSolutionToFile(const TPt <TNodeEDatNet<PASONodeData, PASOEdgeData> >& PNet,
                              const PASOSolution& sol,
                              const std::string& file_name)
{
    std::ofstream ofs(file_name.c_str(), std::ofstream::trunc);
    ...
    for(int i=0; i < path.size()-1; i++)
    {
        int src = path[i];
        int des = path[i+1];
        double time = sol.getTimeI(i);
        double speed = sol.getSpeedI(i);
        double fuel_cost = sol.getFuelCostI(i);
        double fuel_time_fun = PNet->GetEDat(src,des).fuelTimeFun(time);
        double fuel_time_fun_D1 = PNet->GetEDat(src,des).fuelTimeFunD1(time);
        double fuel_time_fun_D2 = PNet->GetEDat(src,des).fuelTimeFunD2(time);  
        //sanity check
        double tol = 1e-10;
        if( time < PNet->GetEDat(src,des).getTimeMin() - tol
        ||  time > PNet->GetEDat(src,des).getTimeMax() + tol
        ||  speed < PNet->GetEDat(src,des).getSpeedMin() - tol 
        ||  speed > PNet->GetEDat(src,des).getSpeedMax() + tol 
        ||  std::abs(time*speed - PNet->GetEDat(src,des).getDistance()) > tol
                         	...
                         	std::vector<unsigned>head) 
{
    ...
    auto tail = load_vector<unsigned>("tail_file");
    ...
    unsigned node_count = latitude.size();
    ...
    */
	...
	timer1 -= get_micro_time();
	int ret = PASOUtil::getShortestPath_cch(PNet, source, sink, "new_weight", path_v, new_weight_cost,cch,tail,head,lambda);
                            ...
                            const std::string& file_name)   
{
    ...
    for(double lambda = 0.0; lambda <= 70.0; lambda = lambda + 0.01)
    {
        std::vector<int> path_v;
        std::vector<double> time_v;
...
bool 
PASOUtil::testProcedure(TPt <TNodeEDatNet<PASONodeData, PASOEdgeData> >& PNet,
                        int source,
                        int sink,
                        double T,  
                        double L,
...
/*********************************** time and memory usage ****************************/
    ...
    for (TNodeEDatNet< PASONodeData, PASOEdgeData >::TEdgeI EI = PNet->BegEI(); EI < PNet->EndEI(); EI++) 
    {
      ...
      if(src == 69252)
      {
        // std::cout << "quantized (" << src << "," << des <<"):" << std::endl;
    ...
    std::vector< std::vector<int> > prev_node(max_node_id);
    std::vector< std::vector<double> > prev_time(max_node_id);
    bool doFind = false;
    for(TNodeEDatNet<PASONodeData, PASOEdgeData>::TNodeI ni = PNet->BegNI(); ni != PNet->EndNI(); ni++)
    {
        int node_id = ni.GetId();
        ...
        prev_node[node_id] = std::vector<int>(N+1,-1);
        prev_time[node_id] = std::vector<double>(N+1,-1);
    }
    for (int c=1; c<= N; c++)
    {
            ...
            for(int n=0; n < ni.GetInDeg(); n++)
            {
                ...
                for(int i=0; i < quantized_value.size(); i++)
                {
                    ...
                    if(value <= c)
                    {
                        ...
                        {
#if 0
                        //    if(node_id == 59395 || node_id == 59394 || node_id == 59392)
                            {
                            std::cout << "update for c=" << c 
                                      ...
                                      << ", [prev_node_id][c-value]=" << g[prev_node_id][c-value]
                                      <<", g[prev_node_id][c-value]+time=" << g[prev_node_id][c-value]+time
                                      << ", g[node_id][c]=" << g[node_id][c] 
                                      << std::endl;
                            }
#endif
                            prev_node[node_id][c] = prev_node_id;
                            prev_time[node_id][c] = time;
        ...
        if(g[sink][c] <= T)
        {
            ...
            for(TNodeEDatNet<PASONodeData, PASOEdgeData>::TNodeI ni = PNet->BegNI(); ni != PNet->EndNI(); ni++)
            {
                ...
                for(int i = 0; i <= c; i++)
                {
                    if(g[node_id][i] <= T)
                    {
                        std::cout << "g[" << node_id << "][" << i << "]=" << g[node_id][i] << std::endl;
                    }
                }
            }
#endif    
            ...
            int u = sink;
            int value = c;
            double time = prev_time[sink][value];
            double sum_time = 0;
            while(u != source)
            {
                path_list.push_front(u);
                time_list.push_front(time);
                int v = prev_node[u][value];
                //find the corresponding quantized value for the time     
                const std::vector<int>& quantized_value = PNet->GetEDat(v, u).getQuantizedValue();
                const std::vector<double>& representative_time = PNet->GetEDat(v, u).getRepresentativeTime();
                for(int i=0; i < representative_time.size(); i++)
                {
                    if(std::abs(time-representative_time[i]) <= 1e-10)
                    {
                        value = value - quantized_value[i];
                    }
                }
                std::cout << "u=" << u
                          << ", time=" << time
...
/*********************************** time and memory usage ****************************/     
    ...
    memory_usage_KB_o = memory_usage2-memory_usage1; 
...
int 
PASOUtil::approximationScheme(TPt <TNodeEDatNet<PASONodeData, PASOEdgeData> >& PNet,
                              int source,
                              int sink,
                              double T,  
                              double lb,
...
/*********************************** time and memory usage ****************************/
    ...
    double bl = lb;
    double bu = ub;
    double max_memory_usage_KB = 0.0;
    ...
    while (bu/bl > 16)
    {
        ...
        double memory_usage_KB_temp = 0.0;
        bool ret = testProcedure(PNet, source, sink, T, V, V, 1.0, sol,elapsed_time_second_temp,memory_usage_KB_temp);
        sum_elapsed_time_second += elapsed_time_second_temp;
        if(memory_usage_KB_temp > max_memory_usage_KB)
        {
            max_memory_usage_KB = memory_usage_KB_temp;
        }
        if( ret == false)
        {
            bl = V;
    ...
    double memory_usage_KB_step_3 = 0.0;
    testProcedure(PNet, source, sink, T, bl, bu, epsilon, sol_o,elapsed_time_second_step_3,memory_usage_KB_step_3);
    sum_elapsed_time_second += elapsed_time_second_step_3;
    if(memory_usage_KB_step_3 > max_memory_usage_KB)
    {
        max_memory_usage_KB = memory_usage_KB_step_3;
    }
...
/*********************************** time and memory usage ****************************/     
    ...
    std::cout << "max_memory_usage_KB: " << max_memory_usage_KB << std::endl;
/*********************************** time and memory usage ****************************/       
    return 0;    
}
...
/* ========================================= fastest path ===================================================*/   
    ...
    */
    ...
    ret = PASOUtil::getShortestPath_cch(PNet, source, sink, "time_min", path_fastest, minimal_path_hours, cch, tail, head);
    if(ret == -1)
    {
	...
	*/
    ...
    PASOUtil::writeSolutionToFile(PNet, sol_fastest_path_time_min, sol_fastest_path_time_min_file); 
    ...
    PASOUtil::writeSolutionToFile(PNet, sol_fastest_path_opt, sol_fastest_path_opt_file);
...
/* ========================================= shortest path ===================================================*/   
   ///get the shortest path here
    ret = PASOUtil::getShortestPath_cch(PNet, source, sink, "distance", path_shortest, minimal_path_miles, cch, tail, head);
    if(ret == -1)
    {
        std::cout << "cannot find a (" << source << "," << sink << ")" << "-path" << std::endl;
        return -1;
    ...
    PASOUtil::writeSolutionToFile(PNet, sol_shortest_path_time_min, sol_shortest_path_time_min_file);  
    ...
    PASOUtil::writeSolutionToFile(PNet, sol_shortest_path_opt, sol_shortest_path_opt_file);
...
/* ========================================= optimal path ===================================================*/      
    ...
    ret = PASOUtil::optimizeNetworkDual(PNet, source, sink, T, sol_lb, sol_ub,cch, tail, head);
    ...
    PASOUtil::writeSolutionToFile(PNet, sol_lb, sol_lb_file);
    PASOUtil::writeSolutionToFile(PNet, sol_ub, sol_ub_file);
    PASOUtil::writeSolutionToFile(PNet,sol_opt, sol_opt_file);
...
/*********************************** time and memory usage ****************************/     
    ...
    gettimeofday(&time_end ,NULL);
	elapsed_seconds = time_end.tv_sec - time_start.tv_sec;
	std::cout << "Elapsed Time for runOneInstance: " << elapsed_seconds << std::endl; 
/*********************************** time and memory usage ****************************/     
