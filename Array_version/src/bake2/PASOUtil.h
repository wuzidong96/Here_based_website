...
class PASOUtil
{
    ...
    static int getShortestPath_cch(const TPt <TNodeEDatNet<PASONodeData, PASOEdgeData> >& PNet,
                               int source,
                               int sink,
                               const std::string& type,
                               std::vector<int>& path_o,
    ...
    static int writeSolutionToFile(const TPt <TNodeEDatNet<PASONodeData, PASOEdgeData> >& PNet,
    ...
    //  @param memory_usage_KB_o output the memory usage in KB during  one invoke
    //  @return return true if it can find a solution, return false (fail) otherwise.
    static bool testProcedure(TPt <TNodeEDatNet<PASONodeData, PASOEdgeData> >& PNet,
                              int source,
                              int sink,
                              double T,    
                              double lb,
    ...
    //  @note we have removed const key word for PNet, because the quantized vector in EdgeData 
    //        will be kept updated... Bad design!!! (Interesting things it that even I add const,
    //        it still works..)    
    static int approximationScheme(TPt <TNodeEDatNet<PASONodeData, PASOEdgeData> >& PNet,
                                   int source,
                                   int sink,
                                   double T,    
                                   double lb,
